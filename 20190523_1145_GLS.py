...
def build_generator3(txt_name):
    ...
    x1, x2 = block(d0, x, 16, 5, 1)
    x1, x2 = block(x1, x2, 16, 3, 1)
    x1, x2 = block(x1, x2, 32, 3, 1)
    x1, x2 = block(x1, x2, 32, 3, 1)
...
def build_discriminator2(txt_name):
    ...
    validity = Conv2D(1, kernel_size=5, strides=1, padding='same')(d4)  # (?, 16, 16, 1)
...
if randomTxt == "Rand2":
    ...
    lossWeights = [1, 1, 2, 2, 0.01, 0.01, 15, 15]
    swish_beta = 10.0
...
combined.compile(loss=['mse', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse', 'mse'], loss_weights=lossWeights, optimizer=optimizerC)
