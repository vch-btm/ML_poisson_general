...
def build_generator4(txt_name):
    ...
    x1, x2 = block(x0, x, 64, 3, 1)
    x1, x2 = block(x1, x2, 64, 3, 1)
    # x1 = Concatenate()([x1, x2, bnd_Y])
    x1, x2 = block(x1, x2, 64, 3, 1)
    x1, x2 = block(x1, x2, 64, 3, 1)
    # x1 = Concatenate()([x1, x2, bnd_Y])
    x1, x2 = block(x1, x2, 64, 3, 1)
...
def train(epochs, batch_size=1, sample_interval=50, genABepochs=0, genBAepochs=0, actCount=0):
    ...
    for actEpoch in range(epochs):
        ...
        if epoch % sample_interval == 0:
            ...
            print("{} {}: {} {} {} {} -> {}".format(randomTxt, epoch, loss, v_loss, loss - v_loss, time.ctime(e1), time.ctime(e1 + e_diff)))
...
if randomTxt == "Rand2":
    if use_boundaries:  # 20190603_0959
...
if randomTxt == "GLS":  # 20190523_1145 or 20190621_0051
    g_AB = build_generator3("g_AB")
    g_BA = build_generator3("g_BA")
    optimizerD = Adam(0.00002, 0.75)
    optimizerC = Adam(0.00002, 0.75)
    optimizerG = Adam(0.0002)  # , 0.5)
    lossWeights = [1, 1, 10, 10, 0.01, 0.01, 50, 50]
    swish_beta = 0.75

if randomTxt == "RandSQ2":  # 20190624_1752
    g_AB = build_generator3("g_AB")
    g_BA = build_generator3("g_BA")
    optimizerD = Adam(0.00002, 0.5)
    optimizerC = Adam(0.00002, 0.5)
...
if randomTxt == "VOR":  # 20190621_0052
    g_AB = build_generator3("g_AB")
    g_BA = build_generator3("g_BA")
    optimizerD = Adam(0.00002, 0.5)
    optimizerC = Adam(0.00002, 0.5)
